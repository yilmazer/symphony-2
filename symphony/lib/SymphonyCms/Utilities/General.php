...
use \SymphonyCms\Crytography\MD5;
use \SymphonyCms\Crytography\SHA1;
...
class General
{
     ...
     */
    public static function reverseSanitize($str)
    {
    ...
    public static function realiseDirectory($path, $mode = 0755, $silent = true)
    {
        ...
        try {
            ...
            if ($silent === false) {
                throw new Exception(tr('Unable to create path - %s', array($path)));
            }
    ...
    public static function deleteDirectory($dir, $silent = true)
    {
        try {
            ...
            if ($silent === false) {
                throw new Exception(tr('Unable to remove - %s', array($dir)));
            }
    ...
    public static function inArrayMulti($needle, $haystack)
    {
        ...
        if (is_array($haystack)) {
            foreach ($haystack as $key => $val) {
                ...
                if (is_array($val)) {
                    if (self::inArrayMulti($needle, $val)) {
                        return true;
                    }
     ...
     * of both. If there is no mergeFilePostData function defined then
    ...
    {
        if (!function_exists('mergeFilePostData')) {
            function mergeFilePostData($type, array $file, &$post)
            {
                foreach ($file as $key => $value) {
                    ...
                    if (is_array($value)) {
                        mergeFilePostData($type, $value, $post[$key]);
                    } else {
                        $post[$key][$type] = $value;
                    }
                }
        ...
        foreach ($files as $type => $data) {
            mergeFilePostData($type, $data, $post);
        }
     ...
     */
    public static function arrayFindAvailableIndex($array, $seed = null)
    {
        if (!is_null($seed)) {
            $index = $seed;
        } else {
     ...
     */
    public static function arrayRemoveDuplicates(array $array, $ignore_case = false)
    {
        return ($ignore_case == true ? self::arrayIunique($array) : array_unique($array));
    }
     ...
     *  a new array containing only the unique elements of the input array.
     */
    public static function arrayIunique(array $array)
    {
        $tmp = array();

        foreach ($array as $key => $value) {
            if (!self::inIarray($value, $tmp)) {
                $tmp[$key] = $value;
            }
        }
     ...
     */
    public static function arrayMapRecursive($function, array $array)
    {
        $tmp = array();

        foreach ($array as $key => $value) {
            if (is_array($value)) {
                $tmp[$key] = self::arrayMapRecursive($function, $value);
            } else {
     ...
     */
    public static function arrayToXml(XMLElement $parent, array $data, $validate = false)
    {
    ...
    public static function deleteFile($file, $silent = true)
    {
        try {
            ...
            if ($silent == false) {
                throw new Exception(tr('Unable to remove file - %s', array($file)));
            }
    ...
    public static function ensureType(array $params)
    {
        foreach ($params as $name => $param) {
            ...
            if (!function_exists($validator)) {
                throw new InvalidArgumentException(tr('Enforced type `%1$s` for argument `$%2$s` does not match any known variable types.', array($param['type'], $name)));
            }

            // validate variable type
            if (!call_user_func($validator, $param['var'])) {
                throw new InvalidArgumentException(tr('Argument `$%1$s` is not of type `%2$s`, given `%3$s`.', array($name, $param['type'], gettype($param['var']))));
            }
        }
    }
